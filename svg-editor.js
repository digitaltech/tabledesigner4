...
(function() {
	...
	if (!window.svgEditor) window.svgEditor = function($) {
		var svgCanvas,
			Editor = {},
			isReady = false,
			//filepath = '/OnlineTablesDesigner',  //Harcoded!!!
			filepath = '/tabledesigner',
			schema = 'public',
			defaultPrefs = {
				lang: 'en',
				iconsize: 'm',
			...
			uiStrings = Editor.uiStrings = {
				...
				layers: {
					layer: 'Floors/Layouts'
				},
				notification: {
					invalidAttrValGiven: 'Invalid value given',
					noContentToFitTo: 'No content to fit to',
					dupeLayerName: 'There is already a floor named that!',
					enterUniqueLayerName: 'Please enter a unique floor name',
					enterNewLayerName: 'Please enter the new floor name',
					layerHasThatName: 'Floor already has that name',
					QmoveElemsToLayer: 'Move selected elements to floor \'%s\'?',
		...
		Editor.init = function() {
			...
			$.svgIcons(curConfig.imgPath + 'svg_edit_icons.svg', {
				...
				placement: {
					...
					'#tool_source': 'source',
					'#guests_add': 'guests_add',
					'#guests_group': 'guests_group',
					'#guests_manage': 'guests_manage',
				...
				resize: {
					...
					'#guests_button .dropdown .svg_icon': 9,
...
// 					}
			...
			(function() {
				...
				var box = $('#dialog_box'),
					...
					dialog_content = $('#dialog_content');
						       //'select', msg,       cb,        txt, opts, changeCb
				var dbox = function(type, msg, callback, defaultVal, opts, changeCb) {
					...
					if (type === 'prompt'){
						ctrl = $('<input id="alertdiv" type="text">');
						ctrl.val(defaultVal || '');
						$('#dialog_box').append(ctrl);						
						$dialog.dialog('option', 'buttons', {
                       		"Ok" : function() {
                       			$(this).dialog("close");
    							if (callback) callback(ctrl.val());
                       		},
                       		"Cancel" : function() {
                       			$(this).dialog("close");
                       			callback(false);
			...
			// Return default path
			var path = Editor.path = function(){
				return Editor.filepath;
			};
			// Return default dbname
			var dbschema = Editor.schema = function(){
				return Editor.schema;
			};
			
			function ajax_getLayout(id){
				var $sellayout = {
						"table": 'layouts', 
			...
			function ajax_updateLayout(id,vid,title,w,h,svg){
				var $data = {
						"table": 'layouts', 
						"setfields": 'id,venue_id,name,height,width,svg',
						"setvalues": [
						              id,
						              vid,
						              title,
			...
			function ajax_updateSeats(id,oid,eid){
				var $data = {
						"table": 'seatings', 
						"setfields": 'layout_id,event_id',
						"setvalues": [
						              	id,
						              	eid
						              ],
			...
			function ajax_createLayout(vid,title,event,w,h,svg){
				var $data = {
						"table": 'layouts', 
						"setfields": 'venue_id,name,event_id,height,width,svg',
						"return": 'layout_id',
						"setvalues": [
						              vid,
						              title,
				...
				return $.ajax({
					url: path() + "/insert_simpledata.php",
					type: "POST",
					beforeSend: function (){
	            		$('#myoverlay').removeClass('hidden');
	            		$('#myoverlay').addClass('ajaxloader');
			...
			var getSelectDataFromAjax = Editor.getSelectDataFromAjax = function($url, $data, cb){
				var tmp = undefined;
				return $.ajax({
					url: $url,
			...
			var setDataFromAjax = Editor.setDataFromAjax = function($url, $data, cb){
				$.ajax({
					url: $url,
					//dataType: "json",
					type: "POST",
			...
			var origEvent = $('event:first').text();
			var origVenue = $('venue:first').text();
				...
				$.each(holders, function(hold_sel, btn_opts) {
					...
					buttons.addClass('tool_button')
						...
						.each(function(i) {
							...
							//var opts = btn_opts[i];
							// fix for handling extension flyout
							var opts = Actions.getButtonData('#'+this.id);
			...
			var extAdded = function(window, ext) {
				...
				if (ext.context_tools) {
					$.each(ext.context_tools, function(i, tool) {
						...
						switch (tool.type) {
						case 'push_button': //add by kc01
							var html = '<div class="push_button" id="' + tool.id + '" title="' + tool.title + '">' + tool.id + '</div>';
						...
						case 'input':
							var html = '<div' + cont_id + '>'
								...
								+ '" size="' + (tool.size || "4") + '" value="' + (tool.defval || "") + '" type="text"/></div>';
...
// 									key: btn.key,
	...
	// 							}
			...
			var updateContextPanel = function() {
				...
				if (elem != null && (new RegExp('TABLE')).test((elem.id).toUpperCase())){
					return; //cannot do anything with this object
				}
...
// 					$('.contextMenu').hide();
				...
				overlay.bind('mousedown',function() {
					if (!button.hasClass('buttondown')) {
						svgCanvas.runExtensions("onMainMenu");
			...
			*/
			...
			var clickSelect = svgCanvas.clickSelect = function() {
			...
			// var mouseclickSelect = svgCanvas.mouseclickSelect = function() {
			// 	if (mousecursorClick('#tool_select')) {
			// 		svgCanvas.setMode('select');
			// 		svgCanvas.runExtensions('modeButtonStateUpdate');
			// 		$('#styleoverrides').text('#svgcanvas svg *{cursor:move;pointer-events:all}, #svgcanvas svg{cursor:default}');
			...
			var deleteSelected = function() {
				var lelems = svgCanvas.getSelectedElems();
				var answer = confirm("Are you sure you want to delete?");
				if (answer){
				if (selectedElement != null || multiselected) {
					svgCanvas.runExtensions('onDeleteElement', {
						selectedElement: selectedElement,
						multiselected: multiselected,
						selectedElements: lelems
					});
			...
			var clickClear = function() {
				...
				$.confirm(uiStrings.notification.QwantToClear, function(ok) {
					...
					updateTitle("Default");
					svgCanvas.setDocumentTitle("Default");
			...
			var clickExport = function() {
				$.select('Select an image type for save: ', [
			...
			var showDocProperties = function() {
				if (docprops) return;
				docprops = true;
				//Made the form dynamic because we need to handle not only the title, but also venues and events. And those are from server side data.
				var docprops_form = '	<div id="">' +
									'		<fieldset id="svg_docprops_docprops">' +
									'			<fieldset id="image_props">' +
									'				<legend id="svginfo_image_props">Required Properties</legend>' +
									'				<label for="canvas_venue">Venue:</label>' +
									'				<select id="canvas_venue"></select>' +
									'				<br/>' +
									'				<label for="canvas_event">Event:</label>' +
									'				<select id="canvas_event"></select>' +
									'				<br/>' +
									'				<label for="canvas_layout">Layout:</label>' +
									'				<input id="canvas_layout" type="text" value="layout_1"/>' +
									'				<br/>' +
//									'				<label for="canvas_title">Title:</label>' +
//									'				<input type="text" id="canvas_title"/><br/>' +
									'			</fieldset>' +
               ...
               .dialog({
                   ...
                   open: function(){
                	   	$('.ui-widget-overlay').addClass('custom-overlay');
						var max = 0;
						$("#image_props label").each(function(){
						    if ($(this).width() > max)
						        max = $(this).width();   
						});
						$("#image_props label").width(max);
						$("#image_props label").css("display", "inline-block");
						$('#canvas_venue').width(280);
						$("#image_props label").css("text-overflow", "ellipsis");
						$('#canvas_event').width(280);
						$("#image_props label").css("text-overflow", "ellipsis");
						//load venues select
						var $data = {
										"table": 'venues', 
						...
						getSelectDataFromAjax(path() + "/get_simpledata.php",
												$data,
												function (response) {
													var $html = '';			    											
													//load the select on callback
												    var len = response.d.length;
												    for (var i = 0; i< len; i++) {
												        $html += '<option value="' + response.d[i][0] + '">' + response.d[i][1] + '</option>';
												    }
												    //get the currently selected venue
												    $('#canvas_venue').append($html);
												    $('#canvas_venue > option[value="' + svgCanvas.getDocumentVenue() + '"]').attr("selected", "selected");
												    //$('#canvas_venue').val(svgCanvas.getDocumentVenue());
								            		$('#myoverlay').removeClass('ajaxloader');
								            		$('#myoverlay').addClass('hidden');
												});					    
						//load events select
						...
						getSelectDataFromAjax(path() + "/get_simpledata.php",
											...
											function (response) {
											    ...
											    //get the currently selected venue
											    $('#canvas_event').append($html);
											    $('#canvas_event > option[value="' + svgCanvas.getDocumentEvent() + '"]').attr("selected", "selected");
											    //$('#canvas_event').val(svgCanvas.getDocumentVenue());
							            		$('#myoverlay').removeClass('ajaxloader');
							            		$('#myoverlay').addClass('hidden');
											});
                   },
				...
				$('#canvas_layout').val(svgCanvas.getDocumentTitle());				//restore the selected venue & event
...
// 				}
			...
			var updateEvent = function(event) {
				event = event || svgCanvas.getDocumentEvent();
				var newEvent = origEvent + (event ? ': ' + event : '');
				$('event:first').text(newEvent);
			};


			var updateVenue = function(venue) {
				venue = venue || svgCanvas.getDocumentVenue();
				var newVenue = origVenue + (venue ? ': ' + venue : '');
				$('venue:first').text(newVenue);
			};

			var saveDocProperties = function() {				
				// set event
				var oe = svgCanvas.getDocumentEvent();
				var newEvent = $('#canvas_event').val();
				updateEvent(newEvent);
				svgCanvas.setDocumentEvent(newEvent);

				// set venue
				var newVenue = $('#canvas_venue').val();
				updateVenue(newVenue);
				svgCanvas.setDocumentVenue(newVenue);
				...
				// set title				
				var nt = $('#canvas_layout'), newTitle = nt.val();
				if (newTitle === null || newTitle.match(/^ *$/) !== null){
					$.alert(uiStrings.notification.invalidAttrValGiven);
					nt.parent().addClass('error');
					return false;
				}
				nt.parent().removeClass('error');
				updateTitle(newTitle);
				svgCanvas.setDocumentTitle(newTitle);
				// try to update the DB
				var test = true;
				if (svgCanvas.getDocumentId() == "") {
					console.log("Unable to find this layout, will create it.");
					$.when(ajax_createLayout(newVenue,newTitle,newEvent,w,h,svgedit.utilities.encode64(svgCanvas.getSvgString()))).done(function(b1){
						response = b1;
						if (response && response.status === 0) {
							console.log("Layout successfully created.");								
							svgCanvas.setDocumentId(response.d[0][0]);
							$.when(ajax_updateLayout(svgCanvas.getDocumentId(),newVenue,newTitle,w,h,svgedit.utilities.encode64(svgCanvas.getSvgString()))).done(function(b1){
								response = b1;
								if (response && response.status === 0) {
									//Update the layout with the new id set in svg
									console.log("Layout ID set to " + svgCanvas.getDocumentId());								
								} else {									
									console.log("Layout ID update failed.");								
							...
							console.log("Layout creation failed.");	
							svgCanvas.removeDocumentEvent();
							svgCanvas.removeDocumentVenue();
							var re = new RegExp('.*duplicate.*');
			    			$res = response.responseText.match(re);
		            		$('#myoverlay').removeClass('ajaxloader');
		            		$('#myoverlay').addClass('hidden');
			    			if (!!$res) {
								$.alert('Error: The layout name already exists in the database.'); 			    				
			    			} else {
								$.alert('Error:' + response.responseText); 			    				
			    			}
							test = false;
					...
					console.log("Layout found, will update it.");				
					$.when(ajax_updateLayout(svgCanvas.getDocumentId(),newVenue,newTitle,w,h,svgedit.utilities.encode64(svgCanvas.getSvgString()))).done(function(b1){
						response = b1;
						if (response && response.status === 0) {
							console.log("Layout successfully updated.");								
							//need to update seats that have been already assigned to a different event on the current layout.
							if (oe !== ""){
								$.when(ajax_updateSeats(svgCanvas.getDocumentId(),oe,newEvent)).done(function(c1){
									response = c1;
									if (response && response.status === 0) {
										console.log("Seats successfully updated.");		
					            		$('#myoverlay').removeClass('ajaxloader');
					            		$('#myoverlay').addClass('hidden');
									} else {
										$.alert('Error:' + response.responseText); 
...
// 					}
			...
			(function() {
				...
				$.each(toolnames, function(i,item) {
					//This code if wrong and impact upgrade of jquery
...
// 				var opacity = (picker == 'stroke' ? $('#stroke_opacity') : $('#fill_opacity'));
			...
			function deleteLayer() {
				var current_layer = $(svgCanvas.getCurrentDrawing().getCurrentLayer()).clone();
				if (svgCanvas.deleteCurrentLayer()) {
					svgCanvas.runExtensions("onDeleteLayer", current_layer);
					updateContextPanel();
					populateLayers();
		...
		// 	}
			...
			var Actions = function() {
				//Need to disable the shortcuts because of jqGrid (2013/11/11)
...
//					{sel: '#tool_rect', fn: clickRect, evt: 'mouseup', key: ['R', true], parent: '#tools_rect', icon: 'rect'},
					...
					{sel: '#tool_rect', fn: clickRect, evt: 'mouseup', parent: '#tools_rect', icon: 'rect'},
...
//					{sel: '#tool_ellipse', fn: clickEllipse, evt: 'mouseup', key: ['E', true], parent: '#tools_ellipse', icon: 'ellipse'},
					{sel: '#tool_ellipse', fn: clickEllipse, evt: 'mouseup', parent: '#tools_ellipse', icon: 'ellipse'},
...
//					{sel: '#tool_source_cancel,.overlay,#tool_docprops_cancel,#tool_prefs_cancel', fn: cancelOverlays, evt: 'click', key: ['esc', false, false], hidekey: true},
					{sel: '#tool_source_cancel,.overlay,#tool_docprops_cancel,#tool_prefs_cancel', fn: cancelOverlays, evt: 'click', hidekey: true},
...
//					{sel: '#tool_group', fn: clickGroup, evt: 'click', key: ['G', true]},
					...
					{sel: '#tool_group', fn: clickGroup, evt: 'click'},
...
//					{sel: '#sidepanel_handle', fn: toggleSidePanel, key: ['X']},
				...
				return {
					...
					appendButtonData: function(btn) {
						btn.sel = "#" + btn.id;
						btn.fn = btn.events.click;
						tool_buttons.push(btn);
					},
					last: function() {
						return tool_buttons.slice(-1)[0];
					}
				};
			}();
			...
			window.onbeforeunload = function() {
				svgCanvas.runExtensions('onBeforeUnload');
