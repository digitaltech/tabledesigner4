...
var clearSvgContentElement = canvas.clearSvgContentElement = function() {
	...
	var comment = svgdoc.createComment(" Online Table Designer - http://www.keyconcept01.net/ ");
...
//Take an array of elements as input and will group those with the name passed in param.
var groupElements = this.groupElements = function (elements, groupname, title, grouptype) {
	title = typeof title !== 'undefined' ? title : groupname;
	grouptype = typeof title !== 'undefined' ? grouptype : "";
	var id = groupname.replace(/[^\w]/gi, '');
	var	type = 'g';
...
var encodableImages = {},
	...
	// Boolean indicating whether or not there is a drag to the canvas
	dragstarted = false;
...
// Function to set the started boolean
var setStarted = this.setStarted = function (val){
	started = val;
};

//Function to set the drag boolean
var setDragStarted = this.setDragStarted = function (val){
	dragstarted = val;
};
...
// 							call("selected", [selected]);
	...
	var dblClick = function(evt) {
		...
		if (mouse_target != null && (new RegExp('TABLE')).test((mouse_target.id).toUpperCase())){
			return;			
		}
...
this.deleteCurrentLayer = function() {
	...
	if (current_layer) {
		var batchCmd = new svgedit.history.BatchCommand("Delete Floor");
...
this.getDocumentId = function() {
	return canvas.getId(svgcontent);
};

//Function: setDocumentTitle
//Adds a layoutid element for the document with the given id.
//This is an undoable action
//
//Parameters:
//newtitle - String with the new id
this.setDocumentId = function(newid) {
	var childs = svgcontent.childNodes, doc_id = false, old_id = '';
	
	var batchCmd = new svgedit.history.BatchCommand("Change Image Id");
	
	for (var i=0; i<childs.length; i++) {
		if (childs[i].nodeName == 'layoutid') {
			doc_id = childs[i];
			old_id = doc_id.textContent;
			break;
		}
	}
	if (!doc_id) {
		doc_id = svgdoc.createElementNS(NS.SVG, "layoutid");
		svgcontent.insertBefore(doc_id, svgcontent.firstChild);
	} 
	
	if (newid.length) {
		doc_id.textContent = newid;
	} else {
		// No title given, so element is not necessary
		doc_id.parentNode.removeChild(doc_id);
	}
	batchCmd.addSubCommand(new svgedit.history.ChangeElementCommand(doc_id, {'#text': old_id}));
	addCommandToHistory(batchCmd);
}; // End of layoutid


//Function: getEvent
//Returns the current SVG's event contents
this.getEvent = function(elem) {
	elem = elem || selectedElements[0];
	if (!elem) return;
	elem = $(elem).data('gsvg') || $(elem).data('symbol') || elem;
	var childs = elem.childNodes;
	for (var i=0; i<childs.length; i++) {
		if (childs[i].nodeName == 'event') {
			return childs[i].textContent;
		}
	}
	return '';
...
//Returns the current SVG's event contents
this.getSeatStroke = function(elem) {
	elem = elem || selectedElements[0];
	if (!elem) return;
	elem = $(elem).data('gsvg') || $(elem).data('symbol') || elem;
	var childs = elem.childNodes;
	for (var i=0; i<childs.length; i++) {
		if (childs[i].nodeName == 'seatstroke') {
			return childs[i].textContent;
		}
	}
	return '';
...
//Function: setDocumentSeatStroke
//Adds/updates an SeatStroke element for the document with the given stroke.
//The seat stroke is used to identify a free seat
//This is an undoable action
//
//Parameters:
//newStroke - String with the Seat Stroke
this.setDocumentSeatStroke = function(newstroke) {
	var childs = svgcontent.childNodes, doc_stroke = false, old_stroke = '';
	
	var batchCmd = new svgedit.history.BatchCommand("Change Image Seat Stroke");
	
	for (var i=0; i<childs.length; i++) {
		if (childs[i].nodeName == 'seatstroke') {
			doc_stroke = childs[i];
			old_stroke = doc_stroke.textContent;
			break;
		}
	}
	if (!doc_stroke) {
		doc_stroke = svgdoc.createElementNS(NS.SVG, "seatstroke");
		svgcontent.insertBefore(doc_stroke, svgcontent.firstChild);
	} 
	
	if (newstroke.length) {
		doc_stroke.textContent = newstroke;
	} else {
		// No event given, so element is not necessary
		doc_stroke.parentNode.removeChild(doc_stroke);
	}
	batchCmd.addSubCommand(new svgedit.history.ChangeElementCommand(doc_stroke, {'#text': old_stroke}));
	addCommandToHistory(batchCmd);
}; //end of SeatStroke

//Function: getDocumentSeatStroke
//Returns the current document seats stroke or an empty string if not found
this.getDocumentSeatStroke = function() {
	return canvas.getSeatStroke(svgcontent);
};


//Function: getDocumentEvent
//Returns the current document event or an empty string if not found
this.getDocumentEvent = function() {
	return canvas.getEvent(svgcontent);
};

//Function: setDocumentEvent
//Adds/updates an Event element for the document with the given name.
//This is an undoable action
//
//Parameters:
//newEvent - String with the Event
this.setDocumentEvent = function(newevent) {
	var childs = svgcontent.childNodes, doc_event = false, old_event = '';
	
	var batchCmd = new svgedit.history.BatchCommand("Change Image Event");
	
	for (var i=0; i<childs.length; i++) {
		if (childs[i].nodeName == 'event') {
			doc_event = childs[i];
			old_event = doc_event.textContent;
			break;
		}
	}
	if (!doc_event) {
		doc_event = svgdoc.createElementNS(NS.SVG, "event");
		svgcontent.insertBefore(doc_event, svgcontent.firstChild);
	} 
	
	if (newevent.length) {
		doc_event.textContent = newevent;
	} else {
		// No event given, so element is not necessary
		doc_event.parentNode.removeChild(doc_event);
	}
	batchCmd.addSubCommand(new svgedit.history.ChangeElementCommand(doc_event, {'#text': old_event}));
	addCommandToHistory(batchCmd);
}; //end of Events

//Function: removeDocumentEvent
//Remove an Event element for the document.
//This is an undoable action
//
//Parameters:
//None
this.removeDocumentEvent = function() {
	var childs = svgcontent.childNodes;	
	var batchCmd = new svgedit.history.BatchCommand("Remove Image Event");	
	for (var i=0; i<childs.length; i++) {
		if (childs[i].nodeName == 'event') {
...
//Function: getVenue
//Returns the current SVG's venue contents
this.getVenue = function(elem) {
	elem = elem || selectedElements[0];
	if (!elem) return;
	elem = $(elem).data('gsvg') || $(elem).data('symbol') || elem;
	var childs = elem.childNodes;
	for (var i=0; i<childs.length; i++) {
		if (childs[i].nodeName == 'venue') {
			return childs[i].textContent;
		}
	}
	return '';
...
//Function: getDocumentVenue
//Returns the current document venue or an empty string if not found
this.getDocumentVenue = function() {
	return canvas.getVenue(svgcontent);
};

//Function: setDocumentVenue
//Adds/updates a Venue element for the document with the given name.
//This is an undoable action
//
//Parameters:
//newEvent - String with the Venue
this.setDocumentVenue = function(newvenue) {
	var childs = svgcontent.childNodes, doc_venue = false, old_venue= '';
	
	var batchCmd = new svgedit.history.BatchCommand("Change Image Venue");
	
	for (var i=0; i<childs.length; i++) {
		if (childs[i].nodeName == 'venue') {
			doc_venue = childs[i];
			old_venue = doc_venue.textContent;
			break;
		}
	}
	if (!doc_venue) {
		doc_venue = svgdoc.createElementNS(NS.SVG, "venue");
		svgcontent.insertBefore(doc_venue, svgcontent.firstChild);
	} 
	
	if (newvenue.length) {
		doc_venue.textContent = newvenue;
	} else {
		// No event given, so element is not necessary
		doc_venue.parentNode.removeChild(doc_venue);
	}
	batchCmd.addSubCommand(new svgedit.history.ChangeElementCommand(doc_venue, {'#text': old_venue}));
	addCommandToHistory(batchCmd);
}

//Function: removeDocumentVenue
//Remove a Venue element for the document.
//This is an undoable action
//
//Parameters:
//None
this.removeDocumentVenue = function() {
	var childs = svgcontent.childNodes;	
	var batchCmd = new svgedit.history.BatchCommand("Remove Image Venue");	
	for (var i=0; i<childs.length; i++) {
		if (childs[i].nodeName == 'venue') {
			childs[i].parentNode.removeChild(childs[i])
			break;
